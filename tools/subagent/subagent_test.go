...
// holdAgent blocks inside Run until released, so the control test can
...
// waitTurn waits for a child turn to start and pins its prompt.
...
// TestControlTools_SendInterruptList covers the control half of the tool set:
// list_agents, interrupt_agent and send_message act on the shared registry.
func TestControlTools_SendInterruptList(t *testing.T) {
	...
	sub, err := reg.Spawn(toolCtx(), "s1", "c", child, &types.AgentInput{
	...
	// The control tools follow the spawn pair: subagent, subagent_fork,
	// send_message, interrupt_agent, list_agents.
	send, interrupt, list := ts[2], ts[3], ts[4]

	// list_agents shows the live child.
	out, err := list.Run(toolCtx(), "{}")
	...
	if _, err := interrupt.Run(toolCtx(), `{"id":"`+sub.ID+`"}`); err != nil {
		t.Fatalf("interrupt_agent = %v", err)
	}

	// send_message queues the next turn (runs after the interrupted one).
	if _, err := send.Run(toolCtx(), `{"id":"`+sub.ID+`","message":"second"}`); err != nil {
